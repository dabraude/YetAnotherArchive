...
// can handle most things other than the constructor and destructor
...
{                                                                   \
    ...
    try {                                                           \
        ...
        return_type result =                                        \
            static_cast<return_type>(archive->func(__VA_ARGS__));   \
        return result;                                              \
    }                                                               \
    catch(const std::exception& e) {                                \
        std::string msg("exception was raised in YAA_ ");           \
        msg += std::string(#func) + ": ";                           \
        YAA::Log().error(msg + e.what());                           \
        return error_value;                                         \
    }                                                               \
}                                                                   \
